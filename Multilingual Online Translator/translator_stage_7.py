...
if len(argv) != 4:
    ...
    for idx, lang in idx_lang_dict.items():

        if lang.lower() == argv[1]:
            src_lang = idx

        elif None == dst_lang:

            if lang.lower() == argv[2]:
                dst_lang = idx
...
def translate( src_lang, dst_lang, word, mode='one-to-one', stream=stdout):
    ...
    if response.status_code == 200:
        ...
        if response.status_code == 404:
            ...
            raise Exception(f"Something wrong with your internet connection")
...
def task_dispatch( src_lang, dst_lang, word):

    if dst_lang != 0:
        # one-to-one mode, output to console
        translate(src_lang, dst_lang, word, mode = 'one-to-one', stream = stdout)
        ...
        for dst_lang in range(1, len(idx_lang_dict)):
            ...
            translate(src_lang, dst_lang, word, mode = 'one-to-many', stream = file)
...
try:
    if dst_lang is None:
        raise Exception(f"Sorry, the program doesn't support {argv[2]}")
    task_dispatch( src_lang, dst_lang, word)
